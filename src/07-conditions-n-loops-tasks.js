...
function findFirstSingleChar(string) {
  ...
  return stringArr.filter((item) => stringArr.filter((el) => el === item).length === 1)[0] || null;
}
...
function isCreditCardNumber(ccn) {
  return !([...ccn.toString()].reverse()
    .map((item, index) => (index % 2 ? `${item * 2}` : item))
    .map((item) => (item > 9 ? +item[0] + +item[1] : +item))
...
function getDigitalRoot(num) {
  const sum = [...num.toString()].reduce((acc, cur) => +acc + +cur);
...
function isBracketsBalanced(str) {
  ...
  [...str].forEach((item) => {
    if (brackets.has(item)) {
      stack.push(item);
    } else if (brackets.get(stack[stack.length - 1]) === item) {
      stack.pop();
    }
  });
